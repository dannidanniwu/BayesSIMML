...
source("/gpfs/home/dw2625/r/BSIM/Single_bayes_code_06162022.R")
#source("./bsim_code.R")
#source("./Single_bayes_code_06162022.R")
...
single_mod <- cmdstan_model("/gpfs/home/dw2625/r/BSIM/Single_bayes_06162022.stan");
#single_mod <- cmdstan_model("./Single_bayes_06162022.stan");
                            ...
                            single_mod=single_mod)
{
  ...
  data <- generate.data2(n=n, p=p, correlationX=correlationX, sigmaX=sigmaX, m.choice=m.choice, pi.1=pi.1)
  ...
  data$true.eta
  # generate test data
  test.data <- generate.data2(n=n.test, p=p, correlationX=correlationX,sigmaX=sigmaX, m.choice=m.choice, pi.1=pi.1)
  #test.data$y
  xnew <- test.data$X
  ...
  bsim.obj <-  bsim.fn(y,A,X,Xm, family = "binomial", n.samples=n.samples, 
                       ...
                       beta.ini=beta.ini)
  bsim.obj$beta.ini
  mean(bsim.obj$acceptsample)#10*1000(p*n.samples) matrix; each row: posterior samples of beta_i
  ...
  #true.optim.CCP <- table(true.optim.trt)[2]/sum(table(true.optim.trt))
  ...
  sgbayes.coefs <- bayes_single_model(X = X,p=p, y = y, A = A, single_mod = single_mod)
...
# res <- rbindlist(lapply(1:2, function(a) oneExperimentFn(a,n=500, p=5, m.choice="linear", g.choice="linear",
...
for(r in  1:3)
{
  ...
  # res <- replicate(2, oneExperimentFn(iter=2,n=n, p=p, g.choice=g.choice, m.choice=m.choice,
  #                                     n.test= n.test,
...
date_stamp <- gsub("-", "", Sys.Date())
dir.create(file.path("/gpfs/home/dw2625/r/BSIM/", date_stamp), showWarnings = FALSE)
save(results.aggregated2, file = paste0("/gpfs/home/dw2625/r/BSIM/", date_stamp, "/all_3_scenarios.rda"))
